 ...
 * @since March 18 2023
 * @version 0.01.c: start and setup of all files, vars, and methods
 * DESCRIPTION: LDPM
 */
 ...
 * TODO-0.02.a: start implementing the other classes and their functionalities; need to double check toString(), Monster(), and check all other TODO's left in monster.java
 */
...
public abstract class Monster {
    ...
    private static Monster setPhrase(Monster M) {
        if ( M.equals(new WeirdTurtle()) ) {
            M.setPhrase("'Urtle!");
        }
        else if ( M.equals(new FireLizard()) ) {
            M.setPhrase("Deal with it");
        }
        else if ( M.equals(new ElectricRat()) ) {
            M.setPhrase("'Lectric!");
        }
        else if ( M.equals(new FlowerDino()) ) {
            M.setPhrase("Flowah!");
        }
        else {
