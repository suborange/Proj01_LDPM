 ...
 * @since March 18 2023
 * @version 0.03.a: start and setup of all files, vars, and methods
 * DESCRIPTION: LDPM
 */
 ...
 * TODO-0.03.b: need to double check Monster(), and check all other TODO's left in monster.java
 */
...
public abstract class Monster {
    ...
    protected double calculateDefensePoints(Monster M) {
        double defense_roll = Dice.roll(M.defenseMin, M.defenseMax); // roll between min and max ; wait is this right? we roll for attack points and defense points in each monster. wuts this all do?
...
//                }
    ...
    double attack(Monster M) {
        ...
        if (this.isFainted()) {
            ...
            //  double check if this checks the same object or not. \ it DOES!
    ...
    public String toString() {
        if ( this.isFainted() ) {
            ...
            String object_info="";
            object_info+= this.getName() + " has " + this.getHealthPoints() + "/" + MAX_HP;
            object_info+="\nElemental type: [";
            if (elements.size() == 1 ){
